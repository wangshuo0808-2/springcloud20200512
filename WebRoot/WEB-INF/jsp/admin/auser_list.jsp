...
<body>
	...
	<!-- topbar ends -->
		...
		<div class="row-fluid sortable">
				<div class="box span12">
					...
					<div class="box-content">
						<ul class="nav nav-tabs" id="myTab">
							<form class="navbar-search pull-left" action="${pageContext.request.contextPath}/user/list">
								用户名：<input type="text" name="userName" value="${userName2}"/>
								用户类型：<select name="roleId"  id="type">
    									<option value=""  readonly>--请选择--</option>
    									<%--<option value="0" >普通用户</option>
    									<option value="1" >管理员</option>
    									--%><option value="0" ${roleId==0?"selected":"" }>普通用户</option>
    						 			<option value="1" ${roleId==1?"selected":"" }>管理员</option>
    									<%--<c:if test="${roleId==0}"><option value="0" selected>普通用户</option></c:if>		
    									<c:if test="${roleId==1}"><option value="1" selected>管理员</option></c:if>
    									--%></select>
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							...
							<tr>
								...
								<td class="center">${u.gender }</td>
								<td class="center">
									<c:if test="${u.roleId==0}">普通用户</c:if>
									<c:if test="${u.roleId==1}">管理员</c:if>
								</td>
								<td class="center">
									<a class="btn btn-success" href="${pageContext.request.contextPath}/user/detail2?userName=${u.userName}">
    			...
    			<a href="${pageContext.request.contextPath}/user/list?userName=${userName2}&roleId=${roleId}&pageNo=1">首页</a>
    			<a href="${pageContext.request.contextPath}/user/list?userName=${userName2}&roleId=${roleId}&pageNo=${page.pageNo-1}">上一页</a>
    			</c:if>
    			<c:if test="${page.pageNo<=1}">
				<a >首页</a>
				<a >上一页</a>
    			...
    			<a href="${pageContext.request.contextPath}/user/list?userName=${userName2}&roleId=${roleId}&pageNo=${page.pageNo+1}">下一页</a>
    			<a href="${pageContext.request.contextPath}/user/list?userName=${userName2}&roleId=${roleId}&pageNo=${page.totalPageCount}">末页</a>
    			</c:if>
				<c:if test="${page.pageNo>=page.totalPageCount}">
				<a >下一页</a>
				<a >末页</a>
					...
					<div class="box-content">
						<table class="table">
						 ...
						 <%--<tr>
						  ...
						  --%><ul>
							<li><a href="${pageContext.request.contextPath}/user/list?userName=${userName}">Prev</a></li>
							<li class="active">
							  <a href="${pageContext.request.contextPath}/user/list?userName=${userName}">1</a>
							</li>
							<li><a href="${pageContext.request.contextPath}/user/list?userName=${userName}">2</a></li>
							<li><a href="${pageContext.request.contextPath}/user/list?userName=${userName}">3</a></li>
							<li><a href="${pageContext.request.contextPath}/user/list?userName=${userName}">4</a></li>
							<li><a href="${pageContext.request.contextPath}/user/list?userName=${userName}">Next</a></li>
						  </ul>
						</div>     
					</div>
				</div>
